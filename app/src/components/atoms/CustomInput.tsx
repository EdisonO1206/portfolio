...
type InputTypes = "file" | "text" | "number" | "date" | "url" | "email" | "password" | "hidden"
...
const CustomInput = ({ name, className, type, title, onChangeValue, errorMessage, autocomplete = "off", value, isEdit = false, filename } : Props) => {
    return (
        <div className={`relative z-0 w-full mb-5 group col-span-1 ${className}`}>
                ...
                <label htmlFor="dropzone-file" className="flex flex-col items-center justify-center w-full border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 dark:hover:bg-gray-800 dark:bg-gray-700 hover:bg-gray-100 dark:border-gray-600 dark:hover:border-gray-500 dark:hover:bg-gray-600">
                    <div className="flex flex-col items-center justify-center pt-5 pb-6 overflow-hidden">
                        ...
                        {filename && filename.trim() != '' && (
                            <p className="text-xs px-4 mt-4">Archivo seleccionado: {filename}</p>
                        )}
                    </div>
            ...
            {type != 'file' && (
                <>
                    {isEdit ? (
                        <input type={type} autoComplete={autocomplete} value={value ?? ''} name={`${name}`} id={`${name}`}  onChange={onChangeValue} className="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" "  />
                    ) : (
                        <input type={type} autoComplete={autocomplete} defaultValue={value ?? ''} name={`${name}`} id={`${name}`}  onChange={onChangeValue} className="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" "  />
                    )}
                    {type != "hidden" && (
