...
const CustomTextarea = ({ name, className, title, onChangeValue, errorMessage, value } : Props) => {
    return (
        <div className={`relative z-0 w-full mb-5 group col-span-2 ${className}`}>
            <textarea name={`floating_${name}`} id={`floating_${name}`}  onChange={onChangeValue} className="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" defaultValue={value} placeholder=" ">
            </textarea>
